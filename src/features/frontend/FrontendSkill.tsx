...
const FrontendSkill: FC<FrontendSkillProps> = ({ content, title, description, className = "" }) => {
    const [active, setActive] = useState(false);
    
    return <div onClick={() => setActive(state => !state)} className={`${className} group relative p-8`}>
        { content }
        <div 
        style={{
